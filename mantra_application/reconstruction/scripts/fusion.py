...
if __name__ == "__main__":
    ...
    path_save = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    voxel_size = 0.002  # voxel size
    max_depth = 1.2  # max depth
    vol_bnds = np.array([[0, 1.0], [-0.4, 0.4], [-0.5, 0.5]])

    if sys.argv.__len__() > 1:
        path = sys.argv[1]  # get data path
    if sys.argv.__len__() > 2:
        path_save = sys.argv[2]  # get save path
    if sys.argv.__len__() > 3:
        voxel_size = float(sys.argv[3])  # get voxel size
    if sys.argv.__len__() > 9:
        vol_bnds[0][0] = float(sys.argv[4])  # get voxel size
        vol_bnds[0][1] = float(sys.argv[5])  # get voxel size
        vol_bnds[1][0] = float(sys.argv[6])  # get voxel size
        vol_bnds[1][1] = float(sys.argv[7])  # get voxel size
        vol_bnds[2][0] = float(sys.argv[8])  # get voxel size
        vol_bnds[2][1] = float(sys.argv[9])  # get voxel size
    ...
    for image in imagels:
        ...
        prefixls.append(os.path.join(image_dir, base_name.split("_")[0]))
    ...
    for prefix in prefixls:
        ...
        color_image = cv2.cvtColor(cv2.imread(prefix + "_rgb.jpg"), cv2.COLOR_BGR2RGB)
        depth_im = cv2.imread(prefix + "_depth.png", -1).astype(
        ...
        cam_pose = np.loadtxt(prefix + "_pose.txt")  # 4x4 rigid transformation matrix
