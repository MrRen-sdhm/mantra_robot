...
__global__
void Integrate(float * cam_K, float * cam2base, float * depth_im, float * workspace,
  ...
  for (int pt_grid_x = 0; pt_grid_x < voxel_grid_dim_x; ++pt_grid_x) {
    ...
    // Work space filter
    if (pt_base_x < workspace[0] || pt_base_x > workspace[1] || pt_base_y < workspace[2] ||
        pt_base_y > workspace[3] || pt_base_z < workspace[4] || pt_base_z > workspace[5])
      continue;
...
int main(int argc, char * argv[]) {
  ...
  int voxel_grid_dim_x = 240; // 500
  int voxel_grid_dim_y = 320; // 500
  int voxel_grid_dim_z = 280; // 500
  ...
  if (argc > 1) {
    ...
    data_path = argv[1];
    cam_K_file = data_path + "/camera-intrinsics.txt";
    save_path = argv[2];
  }
  ...
  cudaMalloc(&gpu_workspace, 3 * 3 * sizeof(float));
  cudaMemcpy(gpu_workspace, workspace, 6 * sizeof(float), cudaMemcpyHostToDevice);
  ...
  while(true){
    ...
    Integrate <<< voxel_grid_dim_z, voxel_grid_dim_y >>>(gpu_cam_K, gpu_cam2base, gpu_depth_im, gpu_workspace,
  ...
  std::string tsdf_ply_filename = save_path + "/surface_cloud.ply";
  std::string tsdf_bin_filename = save_path + "/tsdf.bin";

  // Compute surface points from TSDF voxel grid and save to point cloud .ply file
  std::cout << "Saving surface point cloud (surface_cloud.ply)..." << std::endl;
  ...
  // ExtractSurfaceVoxel(tsdf_ply_filename, voxel_grid_dim_x, voxel_grid_dim_y, voxel_grid_dim_z,
  //                                 voxel_size, voxel_grid_origin_x, voxel_grid_origin_y, voxel_grid_origin_z,
  //                                 voxel_grid_TSDF, voxel_grid_weight, 0.2f, 0.0f);
