...
// Extract voxel which on the surface
void ExtractSurfaceVoxel(const std::string &file_name, int voxel_grid_dim_x, int voxel_grid_dim_y, int voxel_grid_dim_z,
                                     float voxel_size, float voxel_grid_origin_x, float voxel_grid_origin_y, float voxel_grid_origin_z,
                                     float * voxel_grid_TSDF, float * voxel_grid_weight,
                                     float tsdf_thresh, float weight_thresh) {
  // Create point cloud content for ply file
  for (int i = 0; i < voxel_grid_dim_x * voxel_grid_dim_y * voxel_grid_dim_z; i++) {
    // If TSDF value of voxel is less than some threshold, it's on the surface
    if (std::abs(voxel_grid_TSDF[i]) < tsdf_thresh && voxel_grid_weight[i] > weight_thresh) {
      if (voxel_grid_TSDF[i] > 0)
        voxel_grid_TSDF[i] = 1.0;
      else
        voxel_grid_TSDF[i] = -1.0;
      // continue;
    } else {
